...
class LiveCandle(object):
    ...
    def waitRecentCandles(self,reclevel=0):
        ...
        zoo = [c for c in self.backlog if cmp(lt, c.time)<0 and (c.complete or not self.require_complete)]
        if(len(zoo)==0):
            ...
            if(not self.islive and len(self.backlog)>0):
                ...
                live = [c for c in self.backlog if cmp(c.time, latest.time) >=0 and not c.complete]

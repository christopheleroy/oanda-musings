...
class InstrumentWrapper(object):
    ...
    def commonTimes(self, dim='S5', times=[]):
        cTimes = []
        inx = self.S5inx if(dim not in self.inx)else self.inx[dim]
        for t in times:
            if(t in inx):
                cTimes.append(t)
        return cTimes
    ...
    def dataFor(self, dim='S5'):
        return self.inx[dim] if(dim in self.inx) else self.S5inx
...
class InstrumentStep(object):
    ...
    def atTime(self, dim='S5', t=""):
        ...
        # pdb.set_trace()
        val = inx[t] if(t in inx) else None
        if(val is None):
            return 1
        else:
            # pdb.set_trace()
...
class InstrumentPath(object):
    ...
    def __str__(self):
        ...
        st1 =  "|".join([st.wrapper.instrument.name + ("*" if not st.forward else "") for st in self.path])
        return st1 + " {" + self.ring() + "} [" + str(vol) + "] = " + str(m15) + "/" + str(ln15)


    def ring(self):
        return "_".join([st.pair[0].lower() for st in self.path])

    def bestVolume(self,dim):
        z = [st.wrapper.betterVolume(dim) for st in self.path]
    ...
    def coordinationFactor(self, dim='S5'):
        ...
        facts = list(map((lambda t: reduce(lambda red,s: red*s.atTime(dim,t), self.path, 1)), common))
        l = len(self.path)
...
class PathFinder(object):
    ...
    def findNextStepAlternatives(self, visited, steps):
        ...
        for w in self.wrappers:
            if(w.pair[0] == lastStep and w.pair[1] not in visited):
                alternatives.append(InstrumentStep(w, True))
            elif (w.pair[1] == lastStep and w.pair[0] not in visited):
    ...
    def find(self, seed=None):
        ...
        for p in paths:
            p1 = p[0].pair[0]
            pn = p[len(p)-1].pair[1]
            ws = [w for w in self.wrappers if w.passesThrough(p1) and w.passesThrough(pn)]
            if(len(ws)>0):

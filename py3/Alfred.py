...
class RiskManagementStrategy(object):
    ...
    def parse(desc, robot, rmArgs= {}, bark=""):
        import re
        descs = desc.split(",")                                       # we will follow an example 11:2f7r18p
        them = []
        ...
        for dx in descs:
            ...
            for d in alternatives:                                    # example: 11:2f7r18p
                if(rgx.match(d)):
                    sr = False # support and resistance?
                    lt_x = rgx.match(d).groups()[0]                   # lt_x = 11 (example)
                    if(lt_x.startswith("sr")):
                        lt_x = lt_x[2:]
                        sr=True
                    lt = float(lt_x)                                  # lt = 11  (example)
                    rem = rgx.match(d).groups()[1] # rem: remaining string to parse (substring of d) # rem=2f7r18p
                    mapped = {}
                    while(len(rem)>0):
                        bam = sgx.match(rem)                          
                        if(bam is None):
                            raise ValueError("{} - cannot parse {} as risk-management specs, fails on {}".format(bark, d, rem))
                        xrp = bam.groups()[1]                         # xrp = iteratively f then r then p
                        nnn = float(bam.groups()[0])                  # nnn = iteratively 2 then 7 then 18
                        if(nnn<=0):
                        ...
                        mapped[xrp]=nnn                              # iteratively building object {f:2, r: 7, p:18}
                        st = bam.start()
                        en = bam.end()
                        rem = rem[0:st] + rem[en:]
                    sf = mapped['x'] if('x' in mapped) else (mapped['f'] if('f' in mapped) else 2.0)
                    flipped = 'f' in mapped
                    if('f' in mapped and 'x' in mapped):
                        raise ValueError("{} - parsing {}, cannot specify both x (extend by factor) and f (flip trade)".format(bark, rem))
                    pr = mapped['p'] if('p' in mapped) else (lt/sf)
                    ri = mapped['r'] if('r' in mapped) else (lt/sf)
...
class TradeStrategy(object):
    ...
    def secretSauce(self):
        import numpy as np
        if(self.queue.full()):
            spreads = [x.ask.o - x.bid.o for x in self.queue]
            bids = [x.bid.o for x in self.queue]
            asks = [x.ask.o for x in self.queue]
